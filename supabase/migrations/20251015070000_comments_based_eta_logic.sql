/*
  # Fix ETA Logic - Focus on Comments Column Only
  
  ## Problem Analysis
  The current logic excludes parts based on status, but the real exclusion should be based on 
  the 'comments' column in the parts table. Parts with 'Delivery completed' comments should 
  be excluded because they are already delivered.
  
  ## Solution
  Remove status-based filtering and focus only on comments-based exclusion.
  Only exclude parts where comments = 'Delivery completed' (case-insensitive).
*/

-- Drop and recreate with corrected exclusion logic
...
-- Recreate with comments-only exclusion logic
...
SELECT
  ...
  -- CORRECTED ETA LOGIC: Comments-only exclusion with multiple fallback strategies
  CASE 
    WHEN (qty_from_transit > 0 OR GREATEST(0, quantity_required - qty_from_stock - quantity_used - qty_from_transit - qty_from_invoiced) > 0) THEN
      COALESCE(
        -- Strategy 1: Exact match, exclude only 'Delivery completed' comments
        (
          SELECT MAX(p.eta)
        ...
        -- Strategy 2: Case-insensitive match, exclude only 'Delivery completed' comments
        (
          SELECT MAX(p.eta)
          FROM project_supplier_orders pso
          JOIN parts p ON p.supplier_order = pso.supplier_order
        ...
        -- Strategy 3: Exact match, include ALL parts regardless of comments (last resort)
        (
          SELECT MAX(p.eta)
          FROM project_supplier_orders pso
          JOIN parts p ON p.supplier_order = pso.supplier_order
        ...
        -- Strategy 4: Case-insensitive match, include ALL parts regardless of comments (last resort)
        (
          SELECT MAX(p.eta)
          FROM project_supplier_orders pso
          JOIN parts p ON p.supplier_order = pso.supplier_order
...
-- Create diagnostic view focused on comments
...
SELECT 
  ...
  -- Check for ETA excluding only 'Delivery completed' comments
  (
    SELECT COUNT(*)
    FROM project_supplier_orders pso
    JOIN parts p ON p.supplier_order = pso.supplier_order
  ...
  -- Check for ETA including ALL parts (for comparison)
  (
    SELECT COUNT(*)
    FROM project_supplier_orders pso
    JOIN parts p ON p.supplier_order = pso.supplier_order
  ...
  -- Get sample comments for debugging
  (
    SELECT ARRAY_AGG(DISTINCT p.comments ORDER BY p.comments)
    FROM project_supplier_orders pso
    JOIN parts p ON p.supplier_order = pso.supplier_order
    WHERE pso.project_id = pac.project_id
      AND p.part_ordered = pac.part_number
...
CREATE OR REPLACE FUNCTION verify_comments_based_eta_logic(project_id_param UUID DEFAULT NULL)
RETURNS TABLE(
  part_number TEXT,
  project_id UUID,
...
BEGIN
  ...
  RAISE NOTICE 'Comments-based ETA logic applied successfully';
  RAISE NOTICE 'Only excluding parts with Delivery completed comments';
  ...
  RAISE NOTICE 'Use SELECT * FROM verify_comments_based_eta_logic() to verify results';
END $$;
