/*
  # Simplified ETA Logic - Always Take Longest ETA
  
  ## Requirement Clarification
  Regardless of whether a part is in:
  - Transit only (quantity_in_transit > 0)
  - Backorder only (quantity_missing > 0) 
  - Both Transit and Backorder (quantity_in_transit > 0 AND quantity_missing > 0)
  
  ALWAYS take the LONGEST (furthest) ETA date from ALL available orders for that part.
  
  ## Simplified Logic
  - If part has ANY quantity in transit OR backorder, show ETA
  - Always use MAX(eta) to get the furthest date
  - No need to check specific conditions - just get the longest ETA available
*/

-- Drop and recreate with simplified ETA logic
DROP MATERIALIZED VIEW IF EXISTS mv_project_analytics_complete;

-- Recreate with simplified ETA logic - always take longest ETA
...
-- Create a verification function to test the simplified ETA logic
CREATE OR REPLACE FUNCTION verify_simplified_eta_logic(project_id_param UUID DEFAULT NULL)
RETURNS TABLE(
  part_number TEXT,
  project_id UUID,
...
BEGIN
  ...
  RAISE NOTICE 'Simplified ETA logic migration completed successfully';
  ...
  RAISE NOTICE 'Use SELECT * FROM verify_simplified_eta_logic() to verify the logic';
END $$;
