...
DO $$ 
BEGIN
  -- Check if columns exist and need conversion
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'parts' 
    AND column_name IN ('eta', 'date_cf')
    AND data_type = 'timestamp with time zone'
  ) THEN
    -- Create temporary columns
    ALTER TABLE parts ADD COLUMN IF NOT EXISTS eta_new TEXT;
    ALTER TABLE parts ADD COLUMN IF NOT EXISTS date_cf_new TEXT;
    ...
    ALTER TABLE parts DROP COLUMN IF EXISTS eta;
    ALTER TABLE parts DROP COLUMN IF EXISTS date_cf;
  ...
  ALTER TABLE parts DROP CONSTRAINT IF EXISTS eta_format_check;
  ALTER TABLE parts 
    ADD CONSTRAINT eta_format_check 
    CHECK (eta IS NULL OR is_valid_french_date(eta));
  
  ALTER TABLE parts DROP CONSTRAINT IF EXISTS date_cf_format_check;
  ALTER TABLE parts 
...
DROP TRIGGER IF EXISTS validate_dates ON parts;
