/*
  # Enhanced ETA Logic for Backorder and Transit Parts
  
  ## Requirement Clarification
  The Latest ETA column should:
  1. Check if the part is in Backorder (quantity_missing > 0) OR Transit (quantity_in_transit > 0)
  2. If part is in BOTH Backorder AND Transit, take the LONGEST (furthest) ETA date
  3. If part is only in one status, take the ETA for that status
  4. Use MAX(eta) to ensure we get the furthest date when multiple ETAs exist
  
  ## Current Logic Analysis
  The existing logic uses MAX(p.eta) which is correct, but let's make it more explicit
  and add better documentation to ensure it covers all scenarios.
*/

-- Drop and recreate with enhanced ETA logic
DROP MATERIALIZED VIEW IF EXISTS mv_project_analytics_complete;

-- Recreate with explicit ETA logic for Backorder and Transit
...
SELECT
  ...
  -- ENHANCED ETA LOGIC: Explicit handling of Backorder and Transit scenarios
  (
    SELECT MAX(p.eta)
    ...
    WHERE pso.project_id = allocation_step3.project_id
      ...
      -- Include orders for parts that are in transit OR backorder
      AND (
        -- Part is in transit (has quantity_in_transit > 0)
        allocation_step3.qty_from_transit > 0
        OR
        -- Part is in backorder (has quantity_missing > 0)
        GREATEST(0, allocation_step3.quantity_required - allocation_step3.qty_from_stock - allocation_step3.quantity_used - allocation_step3.qty_from_transit - allocation_step3.qty_from_invoiced) > 0
      )
...
-- Create a verification function to test ETA logic
CREATE OR REPLACE FUNCTION verify_eta_logic(project_id_param UUID DEFAULT NULL)
RETURNS TABLE(
  part_number TEXT,
  project_id UUID,
...
BEGIN
  ...
  RAISE NOTICE 'Enhanced ETA logic migration completed successfully';
  ...
  RAISE NOTICE 'Use SELECT * FROM verify_eta_logic() to verify the logic';
END $$;
